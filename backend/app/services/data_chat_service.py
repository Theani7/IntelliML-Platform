...
class DataChatService:
    ...
    def chat(self, user_message: str, session_id: str = "default") -> Dict[str, Any]:
        ...
        try:
            ...
            response = self._generate_response(user_message, dataset_context, session_id=session_id)
            
            # Add assistant response to history
            history.append({"role": "assistant", "content": response['text']})
            if len(history) > 50:
                self.conversation_history[session_id] = history[-50:]
...
"""
    ...
    def _generate_response(self, user_message: str, dataset_context: str, session_id: str = "default") -> Dict[str, Any]:
...
3. Suggested visualizations if relevant"""}
        ...
        # Add conversation history (limited to last 10 messages per session)
        history = self._get_session_history(session_id)
    ...
    def _execute_code(self, code: str, df: pd.DataFrame) -> tuple:
        ...
        try:
            ...
            # Allow controlled imports for analytics/viz code generated by the assistant.
            ...
            def safe_import(name, globals=None, locals=None, fromlist=(), level=0):
                ...
                if module_root not in allowed_import_roots:
                    raise ImportError(f"Import of '{module_root}' is not allowed in assistant code execution")
    ...
    def _validate_generated_code(self, code: str) -> None:
        """Reject dangerous code patterns before execution."""
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                for alias in node.names:
                    ...
                    if module_root in blocked_modules:
                        raise ValueError(f"Blocked import: {module_root}")
            ...
            if isinstance(node, ast.Call):
                ...
                if isinstance(node.func, ast.Attribute) and node.func.attr in blocked_calls:
                    raise ValueError(f"Blocked function call: {node.func.attr}")
    
    def get_visualization_suggestions(self, session_id: str = "default") -> List[Dict[str, str]]:
