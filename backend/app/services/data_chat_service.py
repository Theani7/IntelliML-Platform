...
class DataChatService:
    ...
    def chat(self, user_message: str, session_id: str = "default") -> Dict[str, Any]:
        ...
        try:
            # Get current dataset
            df = self.data_service.get_dataframe(session_id)
            
            if df is None:
                return {
                    "text": "No dataset loaded. Please upload a file first.",
                    "code": None,
                    "output": None,
                    "visualization": None,
                    "error": True
            ...
            response = self._generate_response(user_message, dataset_context, session_id)
